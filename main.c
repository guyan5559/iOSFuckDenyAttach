...
//iphoneSE2   ios 14.4  kernel
//__TEXT:__const:FFFFFFF00703B224	00000073	C
#define TARGET_KERNELCACHE_VERSION_STRING "@(#)VERSION: Darwin Kernel Version 20.3.0: Tue Jan  5 18:34:51 PST 2021; root:xnu-7195.80.35~2/RELEASE_ARM64_T8030"
...
#define kernel_write32(val, addr) kwrite(&val, addr, sizeof(unsigned int))
...
uint32_t kernel_read32(uint64_t address) {
    int32_t val;
    kread(address, &val, sizeof(int32_t));
    return val;
}
...
int main(int argc, char *argv[]) {
    ...
    if (argc < 2) {
        printf("Usage: pid <integer>\n");
        return 1;
    }

    // 将参数从字符串转换为 int
    int pid_number = atoi(argv[1]);
...
//	 uint64_t versionstraddr = kb + 0x2FFC4;
    //iphoneSE2   ios 14.4  kernel
    uint64_t versionstraddr = kb + 0x37224;
    char versionstr[256];
...
//            uint64_t kernel_proc0 = kernel_read64(kb + 0x2252DB0);
            //iphoneSE2  ios 14.4  kernproc 这里少一层指针 直接读取 0x2914dd8
//            uint64_t kernel_proc0 = kernel_read64(kb + 0x2914dd8);
            struct proc * proc0 =  (struct proc *)malloc(sizeof(struct proc));

            if(kernel_read(kb + 0x2914dd8, proc0, sizeof(struct proc)))
            {
...
//                    int lflagvalue =0;
//                    kernel_read(proc1->p_lflag,lflagvalue,0x8);
                    ...
                    if(ISSET(lflagvalue, P_LNOATTACH))
                    {
                        ...
                        if ((proc1 ->p_uniqueid) ==pid_number) {
                            printf(" !!!P_LNOATTACH set");
//                                CLR(proc1->p_lflag, P_LNOATTACH);
